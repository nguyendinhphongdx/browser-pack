...
export const loginCommand = new Command('login')
  ...
  .action(async (opts) => {
    ...
    try {
      const result = await new Promise<{ token: string; email: string; name: string }>((resolve, reject) => {
        // Create server first, then listen on port 0 to avoid race condition (#4)
        ...
        // Listen on port 0 = OS assigns a free port, no race condition
        server.listen(0, '127.0.0.1', () => {
          ...
          const authUrl = `${serverUrl}/api/auth/google?redirect_uri=${encodeURIComponent(redirectUri)}`;
